...
def run_test_integers():
    ...
    # Test 1:
    expected = [1, 2, 3, 2, 3, 2, 10874979748983898127347]
    answer = integers([[1, 2, 3, 2, 3],
                       [],
                       ['hello', 2, 'here', 'fun', 0.433],
                       ['this', 'should', 'work', 10874979748983898127347]])
    print('Expected is:', expected)
    print('Actual is:  ', answer)
    ...
    for k in range(len(sequence_of_sequences)):
        for j in range(len(sequence_of_sequences[k])):
            if type(sequence_of_sequences[k][j]) == int:
                ints.append(sequence_of_sequences[k][j])
...
def run_test_big_letters():
    ...
    expected = 'SAFCP'
    answer = big_letters([[], [1, 2, 3, 'does it work?'], 'Strings Are Fun', [953], 'CaP'])
    print('Expected is:', expected)
    print('Actual is:  ', answer)
        ...
        if type(sequence_of_sequences[k]) == str:
            for j in range(len(sequence_of_sequences[k])):
                if sequence_of_sequences[k][j].isupper():
                    cap = cap + str(sequence_of_sequences[k][j])
    return cap
